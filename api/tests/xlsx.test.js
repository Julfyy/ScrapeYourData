...
describe('.xlsx tests', () => {
  ...
  test("Glovo Domino's Pizza test on rows count has passed", async () => {
    const workbook = await getXlsx(domino_pizza_config);
    expect(workbook.worksheets[0].rowCount).toBeGreaterThan(0);
  });

  test('Pizza U Jana test on rows count has passed', async () => {
    const workbook = await getXlsx(pizzaujana_config);
    expect(workbook.worksheets[0].rowCount).toBeGreaterThan(0);
  });

  test('Jarosso test on rows count has passed', async () => {
